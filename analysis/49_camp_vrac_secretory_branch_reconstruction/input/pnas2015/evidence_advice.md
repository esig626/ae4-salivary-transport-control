...
Run `digitise_pnas.py` with Pillow/NumPy. It reads original JPEGs, saves all axis calibration and raw pixel selections, and emits CSVs plus visual overlays. Point uncertainty is ±1.5 original pixels; axis-anchor uncertainty is separately declared ±1 pixel. Published SEM caps are digitised separately. Hidden or inseparable SEM is blank, never assigned zero. Figure 5C returns a marker/trace envelope, not a biological SEM. Version 2 expands the image corridor to preserve previously clipped marker pixels; version 1 is retained in `digitisation_v1/`. At 25–75 s the IPR/CCh symbols can overlap and are flagged as ambiguous. The current measurement is voltage clamp; −60 mV is the holding potential.
