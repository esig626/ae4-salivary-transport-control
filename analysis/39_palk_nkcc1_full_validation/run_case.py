"""One gated Task 39 integration per invocation, ordered WT -> 5% -> null.
...
FAILURES = {"wt":"PALK_NKCC1_WT_VALIDATION_FAILED",
    "ae4_5pct":"PALK_NKCC1_AE4_5PCT_FAILED","ae4_null":"PALK_NKCC1_AE4_NULL_FAILED"}
...
def run_case(case):
    ...
    assert budget['source_tests_pass'] and budget['rest_nesting_pass']
    ...
    saved,_,_,_,_,model,y0=models_and_state()
    frozen_wt=json.loads((ROOT/'results/37_wt_nbc_validation/wt_verification.json').read_text())
    assert np.array_equal(y0,frozen_wt['initial_state_vector'])
    ...
    assert hashes==manifest['parameter_hashes']
    ...
    def check_time(*_):
        if budget['numerical_execution_seconds']+time.perf_counter()-START>=900:
            raise ComputeStop('Task 39 cumulative 900-second numerical execution limit')
    ...
    try:
        inspect(0.,y0,'saved_WT_rest_at_exact_onset')
        ...
        status='PASS' if complete else 'PALK_NKCC1_NUMERICAL_SOLVER_FAILED'
    except ScientificStop:
        status=FAILURES[case]
    except ComputeStop as exc:
        status,error='PALK_NKCC1_COMPUTE_BUDGET_STOP',str(exc)
    except Exception:
        status,error='PALK_NKCC1_NUMERICAL_EXECUTION_ABORTED',traceback.format_exc()
    finally:
    ...
    if complete and status=='PASS':
        ...
        secretion={'cumulative_0_600_pL':total,'mean_60_600_pL_s':mean,'endpoint_pL_s':float(q[-1]),
            ...
            'ratio_to_task37_WT_cumulative':total/TASK37_CUMULATIVE,'quadrature':QUADRATURE}
    ...
    verification={'status':status,'case':case,'ae4_expression':CASES[case],
        ...
        'initial_state_bitwise_identical_to_task31_and_task37':True,'last_checked_state_vector':raw_last,
        'core_state_sha256':saved['core_state_sha256'],'genotype':asdict(genotype),
        'only_genotype_difference':diff,'stimulus':asdict(model.stimulus),'parameter_hashes':hashes,
        'scientific_parameters_unchanged':True,'nbc_parameters':asdict(model.nbc_parameters),
        'nhe1_carrier_amount_fmol':inherited.CARRIER,'nkcc1_kinetics':asdict(model.nkcc1_kinetics),
    ...
    budget['status']=('PALK_NKCC1_FULL_VALIDATION_COMPLETE' if case=='ae4_null' and status=='PASS'
