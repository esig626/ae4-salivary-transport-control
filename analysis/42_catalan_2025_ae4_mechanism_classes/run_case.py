"""One gated Task 42 integration per invocation, ordered WT -> 5% -> null.
...
FAILURES = {"wt":"TASK42_SOURCE_CLASS_WT_FAILED",
    "ae4_5pct":"TASK42_SOURCE_CLASS_AE4_5PCT_FAILED","ae4_null":"TASK42_SOURCE_CLASS_AE4_NULL_FAILED"}
...
FLUXES = (
    ...
    ("ae4_hco3_cell_source","ae4_hco3_cell_source_fmol_s",False,"fmol"),
    ("ae4_co3_cell_source","ae4_co3_cell_source_fmol_s",False,"fmol"),
...
def run_case(case):
    ...
    assert budget['started_cases']==preceding
    ...
    def check_time(*_):
        if budget['numerical_execution_seconds']+time.perf_counter()-START>=900:
            raise ComputeStop('Task 42 cumulative 900-second numerical execution limit')
    ...
    def inspect(t,y,kind):
        ...
        # Verify the selected law and frozen multiplier on every monitored state.
        from modern_full_model.nkcc1_palk2010 import palk_cycle_flux_fmol_s
        expected=palk_cycle_flux_fmol_s(row['na_i_mM'],row['k_i_mM'],row['cl_i_mM'],
        ...
        if abs(row['nkcc1_cycle_inward_fmol_s']-expected)>1e-14:
            failures.append({'gate':'Palk_core_active','actual':row['nkcc1_cycle_inward_fmol_s'],'expected':expected})
    ...
    try:
        inspect(0.,y0,'frozen_task42_WT_rest_at_exact_onset')
        ...
        status,error='TASK42_SOURCE_CLASS_COMPUTE_BUDGET_STOP',str(exc)
    except Exception:
        status,error='TASK42_SOURCE_CLASS_NUMERICAL_EXECUTION_ABORTED',traceback.format_exc()
    finally:
    ...
    # Integrate all requested windows before reducing the saved time grid.
    all_windows=[]
    if complete and secretion is not None:
        for lo,hi in ((0,60),(60,600),(0,600)):
            selected=(times>=lo)&(times<=hi)
            for name,field,positive,unit in FLUXES:
                values=np.asarray([r[field] for r in grid])[selected]
                if positive: values=np.maximum(values,0)
                all_windows.append({'quantity':name,'value':float(np.trapezoid(values,times[selected])),
                    'unit':unit,'window_start_s':lo,'window_end_s':hi})
    write_csv(OUT/(case+'_all_window_integrals.csv'),all_windows)
    readouts=[r for r in grid if r['time_s'] in (0.,1e-6,1.) or r['time_s']%5==0]
    ...
    verification={'status':status,'case':case,'ae4_expression':CASES[case],
        ...
        'initial_state_bitwise_identical_to_frozen_task42_wt_rest':True,'last_checked_state_vector':raw_last,
        ...
        'scientific_parameters_unchanged':True,'ae4_routing_model':MODEL_ID,'nbc_parameters':asdict(model.nbc_parameters),
    ...
    if case=='ae4_null':
        all_pass=all(json.loads((OUT/(name+'_verification.json')).read_text())['status']=='PASS'
                     for name in CASES)
        budget['status']=('TASK42_SOURCE_CLASS_VALIDATION_COMPLETE' if all_pass else
                          'TASK42_SOURCE_CLASS_PREDICTIONS_COMPLETE_WITH_FAILURES')
    else:
        budget['status']=case.upper()+'_PASS' if status=='PASS' else status
    budget['no_further_scientific_execution']=case=='ae4_null'
    ...
    write_json(OUT/(case+'_execution.log'),log)
    print(json.dumps({'class':CLASS_ID,'case':case,'status':status,'first_failure':first_failure,'error':error}))
