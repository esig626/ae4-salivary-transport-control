"""One gated Task 40 integration per invocation, ordered WT -> 5% -> null.
...
FAILURES = {"wt":"TASK40_EQUAL_ROUTING_WT_FAILED",
    "ae4_5pct":"TASK40_EQUAL_ROUTING_AE4_5PCT_FAILED","ae4_null":"TASK40_EQUAL_ROUTING_AE4_NULL_FAILED"}
...
def run_case(case):
    ...
    def check_time(*_):
        if budget['numerical_execution_seconds']+time.perf_counter()-START>=900:
            raise ComputeStop('Task 40 cumulative 900-second numerical execution limit')
    ...
    try:
        inspect(0.,y0,'frozen_task40_WT_rest_at_exact_onset')
        ...
        status,error='TASK40_EQUAL_ROUTING_COMPUTE_BUDGET_STOP',str(exc)
    except Exception:
        status,error='TASK40_EQUAL_ROUTING_NUMERICAL_EXECUTION_ABORTED',traceback.format_exc()
    finally:
    ...
    verification={'status':status,'case':case,'ae4_expression':CASES[case],
        ...
        'initial_state_bitwise_identical_to_frozen_task40_wt_rest':True,'last_checked_state_vector':raw_last,
    ...
    budget['status']=('TASK40_EQUAL_ROUTING_VALIDATION_COMPLETE' if case=='ae4_null' and status=='PASS'
