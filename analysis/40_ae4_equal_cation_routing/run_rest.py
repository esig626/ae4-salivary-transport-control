"""One square WT stationary root from the accepted R09 state; no fitting.
...
def main():
    ...
    saved,old_rest,_,model,stim,y31=models_and_reference()
    ...
    assert np.max(np.abs(reconstructed-y31))<1e-12
    ...
    def alarm(*_): raise TimeoutError('Task 40 cumulative numerical execution limit')
    ...
    def residual(z):
        ...
        y=y31 if np.array_equal(z,z0) else state_from_coordinates(model,coordinates(z))
    ...
    result={'prepared_head':PREPARED_HEAD,'branch':BRANCH,'started_utc':datetime.now(timezone.utc).isoformat(),
        'sole_initial_guess_task31_core_sha256':saved['core_state_sha256'],
        'initial_state_vector':y31.tolist(),'initial_coordinates':start.tolist(),
        'coordinate_reconstruction_max_abs':float(np.max(np.abs(reconstructed-y31))),
        ...
        'scientific_parameter_hashes':before,'status':'TASK40_EQUAL_ROUTING_WT_REST_FAILED',
    ...
    try:
        ...
        oldrow=parent.diagnose(old_rest,0.,y31,old_rest.evaluate(0.,y31))[0]
        ...
        result.update(status='PASS' if not failed else result['status'],admissible=not failed,
            ...
            core_state_sha256=sha256_object(y[:12].tolist()),core_shift_from_task31=(y[:12]-y31[:12]).tolist(),
            ...
            observed_shift_from_task31=shifts,flux_ledger=_flux_row(e,model),
    ...
    if result['admissible']:
        files=[ROOT/'src/modern_full_model/ae4_equal_cation_routing.py',
               ROOT/'tests/test_task40_equal_cation_routing.py',*HERE.glob('*.py')]
        write_json(OUT/'frozen_inputs.json',{'prepared_head':PREPARED_HEAD,'branch':BRANCH,
            'scientific_parent':'afd101448763439f369f2682c467f069ea18a442',
            ...
            'task40_execution_files':[{'path':str(p.relative_to(ROOT)),
    ...
    if 'observables' in result:print(json.dumps(result['observables'],indent=2))
