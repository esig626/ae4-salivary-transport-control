"""Coupled NKCC1 ceiling, pump Na affinity, and CaCC stationary design."""
import sys
...
OUTPUT=OUT/'candidate_05'


def amended(template,capacity,pump_na_half,cacc_factor):
    core=template.base_model.base_model
    parameters=replace(core.parameters,membranes=replace(core.parameters.membranes,
        nak_na_half_mM=pump_na_half,g_cl_apical_S=core.parameters.membranes.g_cl_apical_S*cacc_factor))
...
def main():
    ...
    prior=read_json(OUT/'candidate_04/design.json')
    lo=COORDINATE_LOWER[COORD_ROWS];hi=COORDINATE_UPPER[COORD_ROWS]
    initial=np.r_[*[np.log((np.array(s['coordinates'])[COORD_ROWS]-lo)/(hi-np.array(s['coordinates'])[COORD_ROWS])) for s in prior['states']],np.log(prior['capacity_fmol_s']),np.log(prior['pump_na_half_mM']),np.log(.05)]
    calls=0
    def residual(z,details=False):
        nonlocal calls
        calls+=not details
        cap,half,gcl=np.exp(z[16:]);m=amended(template,cap,half,gcl)
        rows=[];states=[]
        ...
        rows.extend([states[1]['observables']['ph_i']-7.28,
            (states[1]['observables']['cl_i_mM']-40.)/40.,
    ...
    try:
        ...
        result={'solver_success':bool(fit.success),'solver_message':str(fit.message),
            ...
            'capacity_fmol_s':float(np.exp(fit.x[16])),'pump_na_half_mM':float(np.exp(fit.x[17])),'cacc_conductance_factor':float(np.exp(fit.x[18])),
            'constraints':{'null_pH':7.28,'null_Cl_i_mM':40.,'null_sustained_flow_deficit':.33},'states':states}
