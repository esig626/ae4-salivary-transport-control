...
FAILURES = {"wt":"TASK41_CAPACITY_WT_FAILED",
    "ae4_5pct":"TASK41_CAPACITY_AE4_5PCT_FAILED","ae4_null":"TASK41_CAPACITY_AE4_NULL_FAILED"}
...
def run_case(case):
    ...
    def inspect(t,y,kind):
        ...
        expected=capacity_limited_cycles(demand,model.nkcc1_capacity.cycle_capacity_fmol_s,genotype.nkcc1_expression)
        if abs(row['nkcc1_cycle_inward_fmol_s']-expected)>1e-14:
            failures.append({'gate':'Palk_demand_with_shared_capacity_active','actual':row['nkcc1_cycle_inward_fmol_s'],'expected':expected})
    ...
    try:
        ...
        status,error='TASK41_CAPACITY_COMPUTE_BUDGET_STOP',str(exc)
    except Exception:
        status,error='TASK41_CAPACITY_NUMERICAL_EXECUTION_ABORTED',traceback.format_exc()
    finally:
    ...
    verification={'status':status,'case':case,'ae4_expression':CASES[case],
        ...
        'noncapacity_scientific_parameters_unchanged':True,'ae4_routing_model':MODEL_ID,'nbc_parameters':asdict(model.nbc_parameters),
    ...
    budget['status']=('TASK41_CAPACITY_VALIDATION_COMPLETE' if case=='ae4_null' and status=='PASS'
